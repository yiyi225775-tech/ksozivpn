...
<head>
    ...
    <style>
        ...
        /* စလစ်ဒီဇိုင်းအသစ် ပေါင်းထည့်ထားသည် */
        .slip-box { 
...
<body>
        ...
        {% else %}
            ...
            <div class="card">
                <form method="post" action="/add" id="addForm">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                        <div><label>ကုန်ဆုံးမည့်ရက် (ပြက္ကဒိန်)</label><input type="date" name="exp_date" id="inDate" onchange="updateSlip()" required></div>
                        <div><label>Port</label><input name="port" value="Auto" readonly></div>
                    </div>
                    <button class="btn btn-p" style="margin-top:15px;">SAVE ACCOUNT</button>
                </form>
    ...
    <script>
        ...
        function updateSlip() {
            ...
            document.getElementById('slipDate').innerText = document.getElementById('inDate').value || '---';
        }
